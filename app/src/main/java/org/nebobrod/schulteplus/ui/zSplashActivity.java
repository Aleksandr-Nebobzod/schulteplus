 ...
 */
@Deprecated
public class zSplashActivity extends AppCompatActivity  implements NetworkConnectivity.ConnectivityCallback {
	public static final String TAG = "zSplashActivity";
						...
						public void onComplete(@NonNull Task<List<AdminNote>> task) {
							if (task.isSuccessful()) {
								...
								if (getVersionCode() <= verDeprecated) {
									...
									showSnackBarConfirmation(zSplashActivity.this,
...
//					animThrob(iv04Network,null);
				...
				default:	// Final step
					...
					if (0 != (testPassedFlags & TEST_TIME_IS_UP)) {
						...
						Toast.makeText(zSplashActivity.this, getString(R.string.msg_tests_failed), Toast.LENGTH_LONG).show();
									...
									new DataRepos<>(UserHelper.class).getLatestUserHelper(intStringHash(user.getUid()))
											.addOnCompleteListener(task -> {
												if (task.isSuccessful()) {
													...
													Toast.makeText(zSplashActivity.this, getString(R.string.err_unknown), Toast.LENGTH_SHORT).show();
													runActivity(SignupActivity.class);
												}
											});
...
//									Log.d(TAG, "run NO verified, dbPref: " + UserDbPreferences.getInstance(ExerciseRunner.getInstance()).getObjectMap().toString() );
									...
									if (0 == (testResFlags & TEST_RES_USER_VERIF_MESSAGE_SHOWN)) {
										showSnackBarConfirmation(zSplashActivity.this, strMessage, new View.OnClickListener() {
											@Override
...
/*	View.OnClickListener rapid (View view) -> {
	...
	private void animStart() {
		...
		testPassedFlags |= TEST_APP_PASSED; // bitwise conjunction to shared flags with test passed
	}
