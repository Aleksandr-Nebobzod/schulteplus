...
| Шаг | Назначение |
|---|---|
| `./gradlew test` (unit), release-сборка, smoke-тест | финальный артефакт |
| Выгрузка в Play Console (внутреннее тестирование → продакшн). Auth остаётся старым — осознанный риск; fallback: трек B2 досрочно | попасть в дедлайн 31.08 |
| ✅ **v117 на Play** | |

> ⚠️ Если выяснится, что старый firebase-auth 16.0.3 не работает на устройстве — авторизацию переписываем сразу на Compose+KMP (трек B2 досрочно, +3–5 дней), двойной работы не делаем.
...
### B2. Авторизация на Compose + KMP — 3–5 дней ⚠️ (главный риск)
| Шаг | Назначение |
|---|---|
...
|---|---|---|
| Старая авторизация (2018) откажет на устройстве/в Play | средняя | fallback: B2 досрочно (+3–5 дней) |
