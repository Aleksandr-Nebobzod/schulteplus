...
- S2. Новый пользователь: старт → (быстрый сплэш) → онбординг (3 слайда) →
  регистрация или демо → Main.
- S3. Забыл пароль: Login → «Забыли пароль?» → email → письмо.
- S4. Демо: «Продолжить без регистрации» (онбординг) или демо-лок в Login → Main
  в демо-режиме.
- S5. Оффлайн: старт без сети → понятное сообщение, формы доступны, вход — после
  восстановления сети.
- S6. Повторный визит: онбординг не показывается (глобальный битфлаг
  `ONBOARDING_SHOWN`); туториалы удалены (TapTarget, D-24).

### 5.3 Требования по экранам (заполняется по решению глубины)
| Экран | Требования (черновик) |
|---|---|
| Splash | стандарт `androidx.core:core-splashscreen` (OS-заставка: AVD-иконка, `setKeepOnScreenCondition`) + статус-экран; анимированная версия (build-переменные) + статус-пиктограммы (интернет, пользователь); 3-сек надпись активации при `emailVerified=false`; ≤ ~1 c (happy path); статус-бар — решение 6.2; реализация — D-29 |
...
| Онбординг | 3 слайда; первый запуск до входа; выбор первого упражнения; глобальный битфлаг `ONBOARDING_SHOWN` (`prf_global`) |
...
**Чек-лист smoke S1–S6 (Фаза 4, на устройстве):**

| # | Проверка | Ожидание |
|---|---|---|
| S1 | Чистый старт с существующей сессией | Сплэш ≤ ~1 c → Main (без Login) |
| S2 | Чистая установка (стереть данные) | Сплэш → онбординг (3 слайда) → регистрация → Main; выбор упражнения на слайде 2 |
| S3 | «Забыли пароль?» → email | Диалог сброса → письмо на почту; snackbar-подтверждение |
| S4 | «Продолжить без регистрации» | Main в демо-режиме (без Firebase-входа) |
| S5 | Авиарежим → вход/регистрация | Snackbar `msg_user_network_failed`, форма доступна; после восстановления сети вход работает |
| S6 | Повторный запуск | Онбординг не показывается (флаг `ONBOARDING_SHOWN`); туториалов нет |
| + | Аналитика | logcat: `auth_splash_shown`, `onboarding_shown`, `auth_login_started/success` (Google — только на релизной подписи, D-23) |
...
1. **Splash** → **B**: брендовая заставка ~600–900 мс с параллельной проверкой Firebase-сессии;
   проверки версии/сети — фоном после входа в Main (D-17). **Уточнение (23.08, D-29):**
   реализация — стандарт `androidx.core:core-splashscreen` (OS-заставка: AVD-иконка,
   `setKeepOnScreenCondition`) + статус-экран (анимированная версия из build-переменных,
   статус-пиктограммы интернет/пользователь, 3-сек надпись активации при
   `emailVerified=false`); поправка: статус сети — на стартовом флоу (пиктограмма).
...
3. **Онбординг** → 3 слайда: (1) что это; (2) выбор упражнения с расходом псикойнов (ExType.price);
   ...
   приложения, до входа** (глобальный битфлаг ONBOARDING_SHOWN; слайд 3 логичен для анонима) (D-19).
...
| Дата | Изменение |
|---|---|
| 2026-08-23 | Splash уточнён по D-29: стандарт core-splashscreen + статус-экран (анимированная версия из build-переменных, пиктограммы интернет/пользователь, 3-сек надпись активации при `emailVerified=false`); §5.3, §6.1 |
| 2026-08-20 | §5.2/5.3 приведены к реализации (онбординг до входа, глобальный флаг, TapTarget удалён); в §5.4 добавлен чек-лист smoke S1–S6 для Фазы 4 |
